...
module.exports = {
    ...
    async execute({ inter }) { 
        ...
        if (!track && !number) inter.reply({ content: `Você tem que usar uma das opções para pular para uma música ${inter.member}... Tentar novamente? ❌`, ephemeral: true });

            if (track) {
        for (let song of queue.tracks) {
            if (song.title === track || song.url === track ) {
                queue.skipTo(song)
                return inter.reply({ content: `Jump para: ${track} ✅` });
            }
        }
    ...
    if (number) {
        ...
        if (!trackname) return inter.reply({ content: `Esta música aparentemente não existe ${inter.member}.  Tentar novamente?❌`, ephemeral: true });   
        queue.skipTo(index);
